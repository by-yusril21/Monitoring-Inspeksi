...
function doPost(e) {
  try {
    ...
    // ========================================================
    // TAMBAHAN BARU: Edit Tanggal di MASTER_JADWAL via Website
    // ========================================================
    if (data.action === "update_master") {
       ...
       if (barisDitemukan !== -1) {
         // Kolom ke-3 adalah TANGGAL AWAL REGREASING
         sheetMaster.getRange(barisDitemukan, 3).setValue(tanggalBaru);
         
         // SANGAT PENTING: Jalankan ulang fungsi rekap agar jadwal 'Sisa Waktu' otomatis terupdate saat tanggal master diganti!
         buatRekapRegreasing(); 
    ...
    if (data.action === "add_email") {
      ...
      // Hitung baris terakhir untuk nomor urut
    ...
    if (data.action === "update_email") {
      ...
      // Cari baris berdasarkan NO urut (Kolom A)
      ...
      if (barisDitemukan !== -1) {
        // Kolom 2: Nama, Kolom 3: Email, Kolom 4: Status
    ...
    // KODE LAMA: Tetap berjalan normal untuk input data harian maintenance
    ...
    // --- KIRIM EMAIL NOTIFIKASI BESERTA HEADERS ---
    kirimNotifikasiEmail(sheetName, "Website/Aplikasi", newRow, headers);
    
    // PANGGIL FUNGSI SORT & RAPIKAN
    urutkanSheet(sheet);
    buatRekapRegreasing(); 
...
// Fungsi helper baru untuk mencari index kolom berdasarkan keyword
...
// FUNGSI UNTUK MENGUBAH TEKS TANGGAL MENJADI OBJECT DATE JS
...
function persiapkanMasterJadwal() {
  ...
  if (!sheetMaster) {
    ...
    sheetMaster.getRange("A1:D1").setValues([["NO", "NAMA MOTOR", "TANGGAL AWAL REGREASING", "FORMAT TANGGAL: DD/MM/YYYY"]]);
    sheetMaster.getRange("A1:D1").setFontWeight("bold").setBackground("#FFC000").setHorizontalAlignment("center");
  }

  var existingData = sheetMaster.getDataRange().getValues();
  var mapMaster = {};
  
  for (var i = 1; i < existingData.length; i++) {
    var motorName = String(existingData[i][1]).trim();
    if (motorName !== "") {
      mapMaster[motorName] = existingData[i][2];
    }
  }
  ...
  for (var j = 0; j < sheets.length; j++) {
    ...
    // KODE BARU (Tambah pengecualian PENGATURAN_EMAIL)
    ...
    if (mapMaster[name] === undefined) {
      lastRow++;
      sheetMaster.getRange(lastRow, 1, 1, 3).setValues([[lastRow - 1, name, ""]]);
      mapMaster[name] = "";
      isAdded = true;
    }
  }
  ...
  if (isAdded) {
    ...
    sheetMaster.getRange(2, 2, lastRow, 1).setHorizontalAlignment("left");
    sheetMaster.getRange(2, 3, lastRow, 1).setHorizontalAlignment("center");
  }
...
// ==================================================================================
// FUNGSI UPDATE REKAP REGREASING (METODE FIXED INTERVAL + EMAIL KEMBALI)
// ==================================================================================
function buatRekapRegreasing() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var mapMaster = persiapkanMasterJadwal();
  ...
  for (var i = 0; i < sheets.length; i++) {
    ...
    for (var j = 0; j < headers.length; j++) {
      var headerText = String(headers[j]).toUpperCase();
      if (headerText.indexOf("REGREASING") !== -1) colRegreasing = j;
      if (headerText.indexOf("TIMESTAMP") !== -1 || headerText.indexOf("WAKTU") !== -1) colWaktu = j;
      if (headerText.indexOf("EMAIL") !== -1) colEmail = j; 
    }

    if (colRegreasing === -1 || colWaktu === -1) {
       outputData.push([noUrut, namaMotor, "ERROR KOLOM", "-", "-", "-", "Kolom Regreasing tidak ditemukan", "-"]);
       noUrut++;
       continue; 
    }
    ...
    for (var r = data.length - 1; r >= 1; r--) {
      var val = String(data[r][colRegreasing]).replace(/(<([^>]+)>)/gi, "").trim().toUpperCase();
      
      if (statusTerbaru === "N/A" && (val === "SELESAI" || val === "BELUM")) {
         statusTerbaru = val; 
         if (colEmail !== -1 && data[r][colEmail]) {
             emailTerbaru = String(data[r][colEmail]).trim(); 
         }
      }
      
      if (val === "SELESAI" && lastSelesaiTime === null) {
         lastSelesaiTime = data[r][colWaktu];
      }
...
// ==================================================================================
// UPDATE: REKAP DATA TERBARU DENGAN KOLOM-KOLOM BARU
// ==================================================================================
function buatRekapDataTerbaru() {
  ...
  // UPDATE: Susunan Header Kolom Data Terbaru
  outputData.push([
    "NAMA MOTOR", 
    "TIMESTAMP", 
    "EMAIL ADDRESS", 
  ...
  for (var i = 0; i < sheets.length; i++) {
    ...
    // Menggunakan fungsi helper getColumnIndexByName yang sudah dipindahkan ke luar loop
  ...
  if (outputData.length > 0) {
    ...
    if (outputData.length > 1) {
      ...
      bodyRange.setHorizontalAlignment("center");
      sheetTerbaru.getRange(2, 1, outputData.length - 1, 1).setHorizontalAlignment("left"); // Nama Motor rata kiri
      sheetTerbaru.getRange(2, outputData[0].length, outputData.length - 1, 1).setHorizontalAlignment("left"); // Kolom Actions rata kiri
    }
...
function getDynamicEmails() {
  ...
  // Jika tabel masih kosong, tetap kirim ke email utama sebagai backup
  return listEmail.length > 0 ? listEmail.join(",") : "Muhsabri.mb@sig.id";
}

// FUNGSI OTOMATIS CEK & BUAT SHEET EMAIL
function getOrCreateEmailSheet() {
  ...
  if (!sheet) {
    // Jika tidak ada, buat sheet baru
    ...
    // Format Header (Warna Biru AdminLTE, Teks Putih, Tebal)
    var headerRange = sheet.getRange("A1:D1");
    headerRange.setBackground("#007bff") // Warna Primary AdminLTE
               .setFontColor("white")
               .setFontWeight("bold")
               .setHorizontalAlignment("center");
    
    // Atur perataan kolom agar rapi
    sheet.getRange("A:A").setHorizontalAlignment("center"); // Kolom No
    sheet.getRange("D:D").setHorizontalAlignment("center"); // Kolom Status
    
    // Berikan contoh data awal agar tidak kosong total
